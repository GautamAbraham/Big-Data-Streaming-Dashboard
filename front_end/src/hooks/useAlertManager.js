...
export function useAlertManager() {
  ...
  const addAlert = useCallback((message, severity = 'warning', location = null) => {
    const now = Date.now();

    // Create location key for duplication prevention
    const locationKey = location ? `${location.lat.toFixed(3)},${location.lon.toFixed(3)}` : 'general';
    ...
    const dismissTime = severity === 'critical' ? 15000 : severity === 'warning' ? 10000 : 7000;
    setTimeout(() => {
